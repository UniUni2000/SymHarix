...
- Claude-compatible 执行链路通过 `scripts/claude-adapter.cjs` 运行。
...
如果 public base URL 是临时 `trycloudflare.com` 地址，HTTP 530、旧 webhook URL、DNS 错误和 pending updates 持续增长应先按隧道层故障处理。先让 `start` 恢复隧道，再考虑改应用代码。
...
- 在本 checkout 中运行 `claude-code/bin/claude-symharix --help`
- 仓库路径可读且 Git `HEAD` 有效
